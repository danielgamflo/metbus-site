document.addEventListener("DOMContentLoaded", function () {
  ...
  const isSubdir = window.location.pathname.includes('/noticias/');
  const prefix = isSubdir ? '../' : '';
  const jsonPath = prefix + 'noticias.json';
  ...
  fetch(jsonPath)
    ...
    .then((noticias) => {
      ...
      // =========================
      // CARRUSEL (index + noticias individuales)
      // =========================
      if (track) {
        const noticiasCarrusel = noticias.slice(0, 9);
